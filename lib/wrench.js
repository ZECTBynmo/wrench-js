 ...
 */
exports.rmdirRecursive = function rmdirRecursive(dir, verifyFile, clbk){
	console.log( "Reading directory " + dir + " recursively" );

    fs.readdir(dir, function(err, files){
		var shouldRemDir = true;
        ...
        (function rmFile(err){
            ...
            if (filename === null || typeof filename == 'undefined') {
				if( shouldRemDir && hasRemovedFiles ) {
					try { return fs.rmdir(dir, clbk); }
					catch( err ) { console.log( err ); return clbk( err ); }
				} else {
					console.log("Returning because we shouldn't remove this directory");
					return clbk();
				}
			}
			...
			if( typeof(file) != "undefined" ) {
				fs.stat(file, function(err, stat){
					...
					if ( verifyFile(file) && stat.isDirectory()) {
						rmdirRecursive(file, verifyFile, clbk);
					} else {	
						console.log( "About to verify!" );
						console.log( file );
						var shouldDelete = verifyFile( file );
						...
						console.log( file + " is not a verified file" );
						shouldRemDir = false;
						rmFile();
					}
				});
